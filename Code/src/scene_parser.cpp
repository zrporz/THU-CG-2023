...
Material *SceneParser::parseMaterial() {
    ...
    Vector3f color(1, 1, 1), diffuseColor(1, 1, 1), specularColor(0, 0, 0), emission(0, 0, 0), type(1, 0, 0);//默认为漫反射
    float shininess = 0, refractive_index = 1;
    getToken(token);
    assert (!strcmp(token, "{"));
    while (true) {
        getToken(token);
    ...
    auto *answer = new Material(diffuseColor, color, emission, type, refractive_index, specularColor, shininess, texturefilename, bumpfilename);
    return answer;
}
...
Sphere *SceneParser::parseSphere() {
    ...
    return new Sphere(center, radius, current_material,v);
}
